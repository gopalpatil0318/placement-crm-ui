          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              {college.enabled_features?.map((feature: string) => (
                <span
                  key={feature}
                  className="px-5 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-700 text-sm font-semibold hover:border-blue-300 transition-colors capitalize cursor-default"
                >
                  {feature.replace("_", " ")}
                </span>
              ))}
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    disabled={toggling}
                    className={`relative inline-flex h-[18px] w-[30px] items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50 ${
                      college.college_status === "active"
                        ? "bg-blue-500"
                        : "bg-slate-300"
                    }`}
...
const StatCard = ({
  ...
  <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden relative group hover:shadow-md transition-shadow">
    <div className="p-6 pb-20 relative z-10">
      ...
      <p
        className={`text-[14px] font-bold mb-4 ${themeColor} opacity-80 uppercase tracking-wide`}
      >
        {label}
      </p>
    ...
    <div className="absolute bottom-0 left-0 w-full leading-[0]">
      <svg
        ...
        className="w-full h-14 opacity-30 group-hover:h-16 transition-all duration-300"
      >
        <path

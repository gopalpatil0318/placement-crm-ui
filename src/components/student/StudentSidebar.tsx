        ...
        items: [
            {
                ...
                subItems: [
                    ...
                    { label: "Update Profile", path: "__update_profile__" },
                ],
            },
        ],
    },
...
export default function Sidebar({ isOpen, onUpdateProfileClick, isUpdateProfileOpen }: { isOpen: boolean; onUpdateProfileClick?: () => void; isUpdateProfileOpen?: boolean }) {
...
function NavItem({
    ...
    const isActive =
        path === activePath ||
        subItems?.some((sub) => sub.path !== "__update_profile__" && sub.path === activePath);
        ...
        <div className="mb-1">
            ...
            {hasSubItems && isExpanded && (
                <div className="ml-9 mt-1.5 space-y-1 relative before:absolute before:left-[-14px] before:top-0 before:bottom-2 before:w-[1px] before:bg-gray-100">
                    {subItems.map((sub) => {
                        // Special handling for Update Profile — navigates to profile page
                        if (sub.path === "__update_profile__") {
                            return (
                                <Link
                                    key={sub.path}
                                    to="/student/profile/personal-info"
                                    onClick={onUpdateProfileClick}
                                    className={`flex items-center gap-2 py-2 px-3 rounded-md transition-all duration-200 group w-full text-left cursor-pointer ${isUpdateProfileOpen
                                        ? "text-blue-600 bg-blue-50/50 font-medium"
                                        : "text-gray-500 hover:text-blue-600"
                                        }`}
                                >
                                    <UserCog size={14} className={`transition-all ${isUpdateProfileOpen ? "text-blue-600" : "text-gray-400 group-hover:text-blue-600"}`} />
                                    {sub.label}
                                </Link>
                            );
                        }

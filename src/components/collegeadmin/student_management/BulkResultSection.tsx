...
export const BulkResultSection = ({
  result,
  onReset,
}: BulkResultSectionProps) => {
  return (
    <div className="mt-8 animate-in fade-in slide-in-from-bottom-4">
      <div
        className={`p-6 rounded-xl border ${result.data?.failed?.length === 0 ? "bg-green-50 border-green-100" : "bg-orange-50 border-orange-100"}`}
      >
        <div className="flex items-start gap-4 mb-6">
          <div
            className={`p-2 rounded-full ${result.data?.failed?.length === 0 ? "bg-green-100 text-green-600" : "bg-orange-100 text-orange-600"}`}
          >
          ...
          <div>
            <h3
              className={`text-xl font-bold ${result.data?.failed?.length === 0 ? "text-green-800" : "text-orange-800"}`}
            >
              {result.message}
            </h3>
            <p className="text-slate-600 mt-1">
              Processed {result.data?.success?.length || 0} successes and{" "}
              {result.data?.failed?.length || 0} failures.
            </p>
          </div>
        </div>
        ...
        {result.data?.failed?.length > 0 && (
          <div className="mb-6 bg-white rounded-xl border border-orange-100 overflow-hidden shadow-sm">
            <div className="bg-orange-50 px-5 py-3 border-b border-orange-100 flex items-center gap-2">
              ...
              <h4 className="font-bold text-orange-800 text-sm uppercase tracking-wider">

...
export const BulkUploadSection = ({
    ...
    return (
        <>
            ...
            <h1 className="text-xl font-semibold text-gray-800 mb-6 tracking-tight">Bulk Student Registration</h1>

            {/* Instructions Box */}
            <div className="bg-blue-50/50 border border-blue-100 rounded-lg p-6 mb-8">
                <div className="flex items-center gap-2 mb-2">
                    <HelpCircle size={16} className="text-blue-700" />
                    <h3 className="font-bold text-blue-800 text-sm uppercase tracking-wider">Instructions</h3>
                </div>
                <p className="text-sm text-slate-500 mb-4 font-medium">Required: name, email, password, department, year.</p>
            ...
            {/* Upload Area */}
            <div className="mt-4">
                <label className={`flex flex-col items-center justify-center border-2 border-dashed rounded-[24px] p-16 cursor-pointer transition-all group min-h-[250px]
                    ...
                    {loading ? (
                        <div className="flex flex-col items-center justify-center animate-pulse">
                            ...
                            <p className="text-slate-500 font-medium">Processing Students...</p>
                        </div>
                    ) : selectedFile ? (
                        ...
                        <>
                            <div className="p-4 bg-gray-50 rounded-full mb-4 group-hover:bg-blue-50 transition-colors">
                                <CloudUpload className="text-gray-300 group-hover:text-blue-400" size={48} strokeWidth={1.5} />
                            </div>
            ...
            {csvData.length > 0 && !loading && (
                <div className="mt-8 overflow-hidden border border-gray-200 rounded-xl animate-in fade-in slide-in-from-bottom-4">
                    <div className="bg-gray-50 px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                        ...
                        <span className="text-xs font-medium text-slate-500 uppercase tracking-wider">{csvData.length} Rows found</span>
                    </div>
                        ...
                        <table className="w-full text-sm text-left text-slate-600">
                            ...
                            <tbody>
                                {csvData.map((row, rowIndex) => (
                                    <tr key={rowIndex} className="bg-white border-b border-gray-100 hover:bg-slate-50 transition-colors">
                                        {headers.map((header, cellIndex) => (
                                            <td key={cellIndex} className="px-6 py-3 whitespace-nowrap font-medium">
                                                {row[header]}
                                            </td>
                                        ))}
                                    </tr>

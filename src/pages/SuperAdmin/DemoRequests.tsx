...
import { useDemoRequests } from "@/hooks/sysadmin/useDemoRequests"
...
const STATUS_OPTIONS = [
    ...
    { value: "demo_scheduled", label: "Scheduled" },
    { value: "demo_completed", label: "Completed" },
...
const TABLE_HEADERS = ["College", "Contact", "Type", "Email", "Phone", "Status", "Date"]
    ...
    return (
        <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm font-medium text-gray-900 dark:text-gray-100">
                {hasFilters ? "No demo requests match your filters" : "No demo requests yet"}
            </p>
            <p className="text-xs text-gray-500 dark:text-gray-400">
                {hasFilters ? "Try adjusting your search or filter criteria." : "Demo requests from the website will appear here."}
            </p>
        </div>
    )
}
...
export default function DemoRequests() {
    const {
        demoRequests, loading, isFetching, error, refresh,
        search, page, limit, pagination, statusFilter, typeFilter,
        handleSearchChange, handleLimitChange, handleStatusFilterChange,
        handleTypeFilterChange, handleNavigateToDetail, setPage,
    } = useDemoRequests()
    ...
    return (
        <AnimatedPage>
            <div className="space-y-8">
                <PageHeader title="Demo Requests" breadcrumbs={BREADCRUMBS} />
                ...
                {error && !loading && (
                    <div className="bg-white dark:bg-gray-900 rounded-xl border border-red-200 dark:border-red-800/50 shadow-sm p-10 flex flex-col items-center gap-4 text-center">
                        ...
                        <div>
                            <p className="font-semibold text-gray-800 dark:text-gray-100 mb-1">Failed to load demo requests</p>
                ...
                {!error && (
                    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm">
                        <Toolbar search={search} statusFilter={statusFilter} typeFilter={typeFilter} limit={limit} onSearchChange={handleSearchChange} onStatusChange={handleStatusFilterChange} onTypeChange={handleTypeFilterChange} onLimitChange={handleLimitChange} />
                        ...
                        <div className="hidden md:block overflow-x-auto">
                            <table className="w-full">
                                ...
                                <DesktopTableBody rows={demoRequests} loading={loading} limit={limit} hasFilters={hasFilters} onRowClick={handleNavigateToDetail} />
                            </table>
                        </div>

                        <div className="md:hidden divide-y divide-gray-100 dark:divide-gray-800">
                            <MobileCardList rows={demoRequests} loading={loading} limit={limit} hasFilters={hasFilters} onCardClick={handleNavigateToDetail} />
                        </div>
...
function Toolbar({ search, statusFilter, typeFilter, limit, onSearchChange, onStatusChange, onTypeChange, onLimitChange }: Readonly<{
    search: string; statusFilter: string; typeFilter: string; limit: number
    onSearchChange: (e: React.ChangeEvent<HTMLInputElement>) => void; onStatusChange: (v: string) => void; onTypeChange: (v: string) => void; onLimitChange: (v: number) => void
}>) {
    return (
        <div className="p-5 border-b border-gray-100 dark:border-gray-800">
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                ...
                <select value={typeFilter} onChange={(e) => onTypeChange(e.target.value)} className="rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none">
...
function DesktopTableBody({ rows, loading, limit, hasFilters, onRowClick }: Readonly<{
    rows: DemoRequest[]; loading: boolean; limit: number; hasFilters: boolean; onRowClick: (id: string) => void
}>) {
    if (loading) {
        return (
            <tbody>
                {Array.from({ length: limit }).map((_, i) => (
                    <tr key={`dr-sk-${String(i)}`} className="border-b border-gray-50 dark:border-gray-800 animate-pulse">
                        {Array.from({ length: TABLE_HEADERS.length }).map((_, j) => (
                            <td key={`dr-sk-${String(i)}-${String(j)}`} className="px-5 py-3.5"><div className="h-4 bg-gray-200 dark:bg-gray-700 rounded w-3/4" /></td>
                        ))}
                    </tr>
                ))}
            </tbody>
    ...
    return (
        <AnimatedTableBody>
            {rows.map((dr) => (
                <AnimatedRow key={dr.id} onClick={() => onRowClick(dr.id)} className="border-b border-gray-50 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 cursor-pointer transition-colors">
                    <td className="px-5 py-3.5">
                        <span className="text-sm font-medium text-blue-600 dark:text-blue-400">{dr.college_name}</span>
                        {dr.city && <p className="text-xs text-gray-400 mt-0.5">{[dr.city, dr.state].filter(Boolean).join(", ")}</p>}
                    </td>
                    <td className="px-5 py-3.5">
                        <span className="text-sm text-gray-900 dark:text-gray-100">{dr.contact_person}</span>
                        <p className="text-xs text-gray-400">{dr.designation}</p>
                    </td>
                    <td className="px-5 py-3.5 text-sm text-gray-600 dark:text-gray-400">{dr.college_type}</td>
                    <td className="px-5 py-3.5 text-sm text-gray-600 dark:text-gray-400">{dr.email}</td>
                    <td className="px-5 py-3.5 text-sm text-gray-600 dark:text-gray-400 tabular-nums">{dr.phone}</td>
                    <td className="px-5 py-3.5"><StatusBadge status={dr.status} /></td>
                    <td className="px-5 py-3.5 text-sm text-gray-500 dark:text-gray-400 tabular-nums">{formatDate(dr.created_at)}</td>
                </AnimatedRow>
            ))}
        </AnimatedTableBody>
    )
...
function MobileCardList({ rows, loading, limit, hasFilters, onCardClick }: Readonly<{
    rows: DemoRequest[]; loading: boolean; limit: number; hasFilters: boolean; onCardClick: (id: string) => void
}>) {
    if (loading) {
        return (<>{Array.from({ length: limit }).map((_, i) => (
            <div key={`dr-mob-sk-${String(i)}`} className="p-4 space-y-3 animate-pulse">
                ...
                <div className="flex gap-3"><div className="h-3 bg-gray-100 dark:bg-gray-800 rounded w-24" /><div className="h-3 bg-gray-100 dark:bg-gray-800 rounded w-20" /></div>
            </div>
        ))}</>)
    }
    ...
    return (
        <>{rows.map((dr) => (
            <button type="button" key={dr.id} onClick={() => onCardClick(dr.id)} className="w-full text-left p-4 hover:bg-gray-50 dark:hover:bg-gray-800/50 cursor-pointer transition-colors active:bg-gray-100 dark:active:bg-gray-800">
                <div className="flex items-start justify-between gap-3 mb-1">
                    <p className="text-sm font-medium text-blue-600 dark:text-blue-400 truncate min-w-0 flex-1">{dr.college_name}</p>
                    <StatusBadge status={dr.status} />
                </div>
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">{dr.contact_person} · {dr.designation}</p>
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500 dark:text-gray-400">
                    <span>{dr.email}</span><span className="tabular-nums">{dr.phone}</span><span>{dr.college_type}</span><span>{formatDate(dr.created_at)}</span>
                </div>
            </button>
        ))}</>
    )
